...
export default function HeroSection(props) {
  ...
  return (
    <div className='mb-8'>
      <div className='text-3xl font-black text-[#1e3a8a] uppercase tracking-tight mb-2 min-h-[2.5rem]'>
        {texts.length > 1 ? (
          <Typewriter
            texts={texts}
            speed={typeSpeed}
      ...
      {ctaText && (
        <div className='mt-4 flex flex-wrap gap-2'>
          <SmartLink href={ctaLink} className='no-underline'>
            <span className='inline-block border-2 border-[#1e3a8a] rounded-sm shadow-[2px_2px_0px_0px_#1e3a8a] bg-[#fde68a] px-4 py-2 font-black text-xs text-[#1e3a8a] uppercase tracking-wider hover:bg-[#1d4ed8] hover:text-white active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all'>
              {ctaText} →
            </span>
          </SmartLink>
        </div>
      ...
      <div className='mt-6 border-b-[3px] border-[#1e3a8a]' />
    </div>
  )
}
